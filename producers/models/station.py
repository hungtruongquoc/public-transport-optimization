...
class Station(Producer):
    ...
    # Load the value schema for arrival events
    ...
    def __init__(self, station_id, name, color, direction_a=None, direction_b=None):
        ...
        # Import Turnstile here to avoid circular dependency
        ...
        try:
            self.producer.produce(
                ...
                value={
                    ...
                    "prev_station_id": prev_station_id if prev_station_id is not None else -1,
                    "prev_direction": prev_direction if prev_direction is not None else ""
                },
            )
            logger.info(f"Arrival event emitted for station {self.station_id} - train {train.train_id}")
        except Exception as e:
            logger.error(f"Failed to emit arrival event: {e}")
